...
<jsp:attribute name="scriptsEspecificos">
	<script type="text/javascript">
		function remover(id,row) {
			swal({   
				...
				function(){   
					var destino = '<c:url value="/tiposDespesa/delete/"/>' + id;
					$.ajax({
						type : 'POST',
						url : destino,
	...
	<jsp:body>
		<section class="wrapper">
			...
			<a class="btn btn-primary" href="<c:url value="/tiposDespesa/form"/>" 
			...
			<table id="dados" data-id-field="id" data-unique-id="id"
				...
				<tbody>
				<c:forEach var="tiposDespesa" items="${tiposDespesa}">
				<tr id="row_${tiposDespesa.id}">
					<td>${tiposDespesa.id}</td>
					<td>${tiposDespesa.descricao}</td>
					<td>
						<a class="edit btn btn-primary" title="Editar"
							href="<c:url value="/tiposDespesa/form/${tiposDespesa.id}"/>">
							<i class="fa fa-pencil" aria-hidden="true"></i></a>
						<a class="remove btn btn-danger" title="Remover"
							href="javascript:remover(${tiposDespesa.id})">
